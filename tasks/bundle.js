...
export function bundle(callback) {
  let nbBundles = 0;
  const compiler = webpack(WATCH ? config[1] : config); // in watch mode, the compiler should only build the server. The client will be built by webpack-dev-middleware.
